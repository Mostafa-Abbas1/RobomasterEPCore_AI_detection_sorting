...
class SortingZone:
    ...
    def __repr__(self):
        return f"SortingZone(name={self.name}, position={self.position}, {self.current_count}/{self.capacity})"
...
class ZoneManager:
    ...
    def create_zone(self, name: str, position: Tuple[float, float], capacity: int = 10) -> SortingZone:
        """
        ...
        """
        zone = SortingZone(name, position, capacity)
        self.add_zone(zone)
        return zone
